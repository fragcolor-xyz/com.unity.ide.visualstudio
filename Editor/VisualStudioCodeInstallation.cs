...
namespace Microsoft.Unity.VisualStudio.Editor
{
	internal class VisualStudioCodeInstallation : VisualStudioInstallation
	{
		...
		{
#if UNITY_EDITOR_OSX
			return Directory.Exists(path) && Regex.IsMatch(path, ".*(Code|Cursor).*.app$", RegexOptions.IgnoreCase);
#elif UNITY_EDITOR_WIN
			return File.Exists(path) && Regex.IsMatch(path, ".*(Code|Cursor).*.exe$", RegexOptions.IgnoreCase);
#else
			return File.Exists(path) && (path.EndsWith("code", StringComparison.OrdinalIgnoreCase) || path.EndsWith("cursor", StringComparison.OrdinalIgnoreCase));
#endif
		}
		...
		public static bool TryDiscoverInstallation(string editorPath, out IVisualStudioInstallation installation)
		{
			...
			string BaseName = "Visual Studio Code";
			if (editorPath.Contains("Cursor"))
			{
				BaseName = "Cursor";
			}
